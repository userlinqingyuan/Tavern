...
└── images/
    ...
    ├── bar-sm.jpg      # 移动端（≤620px）用的小尺寸背景（长边 2000px，约 285KB）
    ├── bar2-sm.jpg     # 同上（约 438KB；原图 3.0MB / 7.6MB，整张解码是移动端卡顿大头）
    ├── character/      # 原始人物卡图片
    ├── ID/             # 证件照
...
- **手机倾斜**：移动端把手机左右倾斜时，人物卡会朝重力方向**微微倾斜**（最多 ±3°；正/背面倒向一致）。iOS 13+ 会在第一次触摸时申请方向权限，桌面端与「减少动效」偏好下不启用
...
- **页脚导航短条 `.tavern-dock`**：6 个子页面共用的**独立玻璃小短条**——圆角胶囊、宽度随内容、居中、当前页高亮且不可点（`<span class="dock-current">`）。`position: sticky` + `bottom: max(18px, env(safe-area-inset-bottom))`：滚动时悬浮在视口底部，内容不足一屏时贴底。**≤620px 保持单行窄条**，靠左右滑动看全部入口，并在底部显示一条 3px 细滚动条作提示；移动端去掉毛玻璃（sticky + `backdrop-filter` 会在滚动时反复重算，很吃性能）。页面里的固定按钮用 `--dock-space`（桌面 96px / 移动 100px）避让，不会被短条挡住。首页不挂（它的导航是九宫格）。
...
- **移动端性能取舍（2026-09-13）**：① ≤620px 背景换成 `*-sm.jpg` 小图（原图 3MB / 7.6MB，整张解码是卡顿主因）；② 浮尘与天气 canvas 在移动端**隔帧渲染**、粒子数单独降档（dust 40→8、雨 120→36、雪 60→18）；③ 滚动变暗量化到 5% 一档，避免整屏叠加层每帧改色反复重绘；④ 玻璃模糊降到 6px 并**关闭折射层**（每张卡少一层 `backdrop-filter`）；⑤ 短条在移动端去掉毛玻璃；⑥ 倾斜只写一个 `--tilt` 变量，且变化 <0.3° 时不动样式
